...
let EmployeeWithArray: [number, string, boolean] []= [];
function EmployeeWithDefaultVal(id: number, name:string, isPermanent: boolean = false){
    employee = [id,name,isPermanent];
    EmployeeWithArray.push(employee);
}

function UpdateEmployees(id: number, name: string, isPermanent?: boolean){
...
function TypeAssertion() {
    var StringOfIds = EmployeeWithArray.map(employee => [employee[0] as any as string, employee[1], employee[2]]);
